...
describe('Basics', () => {
  ...
  */
  ...
  it('test non-store, sync block-by-block get traversal', async () => {
    ...
    map = await map.set(Uint8Array.from([k, k, 3 << 4]), 'pos2+3')
    const deepKey = Uint8Array.from([k, k, 0])
  ...
  it('test non-store, sync block-by-block keys traversal', async () => {
    ...
    map = await map.set(Uint8Array.from([k, k, 2 << 4]), 'pos2+2')
    map = await map.set(Uint8Array.from([k, k, 3 << 4]), 'pos2+3')
    map = await map.set(Uint8Array.from([k, k, 0]), 'pos2+0')
    const rootBlock = store.load(map.id)
    let currentBlock = rootBlock
    ...
    assert.deepEqual(
      [...traversal.keys()],
      [Uint8Array.from([k, k, 0]), Uint8Array.from([k, k, 1 << 4]), Uint8Array.from([k, k, 2 << 4]), Uint8Array.from([k, k, 3 << 4])])
    assert.deepEqual(
      [...traversal.values()],
      ['pos2+0', 'pos2+1', 'pos2+2', 'pos2+3'])
    assert.deepEqual(
      [...traversal.entries()],
      [{ key: Uint8Array.from([k, k, 0]), value: 'pos2+0' },
        { key: Uint8Array.from([k, k, 1 << 4]), value: 'pos2+1' },
        { key: Uint8Array.from([k, k, 2 << 4]), value: 'pos2+2' },
        { key: Uint8Array.from([k, k, 3 << 4]), value: 'pos2+3' }])
