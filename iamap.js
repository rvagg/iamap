 ...
 * @param {number} options.bitWidth - The number of bits to extract from the hash to form an element index at
 ...
 * @param {number} options.bucketSize - The maximum number of collisions acceptable at each level of the Map. A
...
class Element {
  constructor (bucket, link) {
    this.bucket = bucket
    this.link = link
    assert.strictEqual(bucket == null, link != null)
  }
 ...
 *
 * Use `IAMap.create()` or `IAMap.load()` to instantiate.
 *
 * @class
...
class IAMap {
  ...
  async delete (key) {
    ...
    if (bitmapHas(this.dataMap, bitpos)) { // should be in a bucket in this node
      return findDataElement(this, bitpos, key, async (found, elementAt, element, bucketIndex, bucketEntry) => {
        if (found) {
          let lastInBucket = this.elements[elementAt].bucket.length === 1
          if (this.nodeCount() === 0 && this.entryCount() === this.config.bucketSize + 1) {
            ...
            // if this.depth is 0, we're making a new root element, otherwise it's a temporary
            // throw-away to be unpacked
            let newElements = removeFromBucket(this.elements, bitpos, elementAt, lastInBucket, bucketIndex)
            let newDataMap = this.depth === 0 ? this.dataMap : 0
            if (this.depth === 0 && lastInBucket) {
              newDataMap = setBit(newDataMap, bitpos, 0) // delete from existing datamap
            } else {
              newDataMap = setBit(newDataMap, mask(hash, 0, this.config.bitWidth), 1) // pretend it's depth=0
            }
            // no nodes, depth=0, this could either be the new root or be unpacked
            return create(this.store, this.config, newDataMap, 0, 0, newElements)
          } else {
      ...
      return findNodeElement(this, bitpos, async (elementAt, element) => {
        ...
        if (newChild.nodeCount() === 0 && newChild.entryCount() === this.config.bucketSize) {
          if (this.nodeCount() === 1 && this.entryCount() === 0) {
            // we only had one node to collapse and the child was collapsiable so we have
            // a new root node set to go
            return newChild
          } else {
...
function removeFromBucket (elements, bitpos, elementAt, lastInBucket, bucketIndex) {
  ...
  if (!lastInBucket) {
    ...
    newElements.splice(elementAt, 1, newElement)
  } else {
...
async function collapseNodeInline (node, bitpos, newNode) {
  // extract data elements from a node and merge into ours
  let newElements = node.elements.slice()
  let newElement = new Element([])
  for (let e of newNode.elements) {
    for (let c of e.bucket) { // we can assume they are all buckets, no nodes
      newElement.bucket.push(c)
    }
  }
  // in-bucket sort is required to maintain a canonical state
  newElement.bucket.sort((a, b) => Buffer.compare(a.key, b.key))
